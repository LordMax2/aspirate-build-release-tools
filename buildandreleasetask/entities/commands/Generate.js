...
function Generate() {
    ...
    const inputOutputFormat = tl.getInput('outputFormat', true);
    const inputDisableSecrets = tl.getInput('disableSecrets', true);
    const inputIncludeDashboard = tl.getInput('includeDashboard', true);
    const inputWorkingDirectory = tl.getInput('workingDirectory', false);
    if (inputOutputFormat === undefined)
        throw new Error("outputFormat is missing...");
    if (inputDisableSecrets === undefined)
        throw new Error("disableSecrets is missing...");
    if (inputIncludeDashboard === undefined)
        throw new Error("includeDashboard is missing...");
